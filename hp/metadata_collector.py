...
def get_text_from_web(text_path: str, text_block_with_id: str) -> Tuple[str, str]:
    ...
    try:
        ...
        if not os.path.isfile(text_path) or not os.path.isfile(text_with_headers_path):
            url = (
                TEXT_URL_PREFIX +
                re.search(r'<a href="/works/(\d+)">', text_block_with_id).groups()[0] +
                "?view_adult=true")
    ...
    with codecs.open(path, 'w', encoding="utf8") as csvfile:
        ...
        for signpost_no, filename in enumerate(all_known_files):
            ...
            for no, part in enumerate(parts):
                ...
                try:
                    sublovers = re.search("<li class='relationships'>((?:(?<!</).)*)", part).groups()[0]
                ...
                wordcount = re.search(r'<dd class="words">((?:\d*,?)+)</dd>', part).groups()[0].replace(",", "")
