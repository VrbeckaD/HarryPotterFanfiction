...
def get_text(text_path: str, text_block_with_id: str) -> Tuple[str, str]:
    """Stáhne soubor z AO3, pokud už není stažený. Vrací jak vyfiltrovaný text, tak i HMTL elementy pro extrahování tagů.

    Args:
        text_path: kam se uloží, když ještě není stažený
        text_block_with_id: kus HTML, kde se nachází ID dokumentu


    Returns:
        čistý text + komplet HTML

    """
    text_with_headers_path = str(text_path).replace(".txt", "_full.txt")
    if not os.path.isfile(text_path):  # tady budeme stahovat soubor
        url = "https://archiveofourown.org/works/" + re.search(r'<a href="/works/(\d+)">', text_block_with_id).groups(1)[
            0] + "?view_adult=true"
    ...
    with codecs.open('HP_catalogue.csv', 'w', encoding="utf8") as csvfile:
        ...
        for filename in [str(name) + ".txt" for name in range(1, 4300)]:
            ...
            for no, part in enumerate(parts):
                ...
                text, full_text = get_text(text_path, part)
